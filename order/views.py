...
def addtoshopcart(request):
    ...
    if cart: #an existing
        ...
        if prodchecker: #product exists in the cart
        ...
        newcart.order_code = ordernumber
...
def checkout(request):
    ...
    profile = UserProfile.objects.get(user__username=request.user.username) #we are picking from the user model
    randomize = Product.objects.filter(featured=True)[:1]
    total = 0
    shippingfee = 0
    vat = 0
...
def placeorder(request):
    api_key ='sk_test_d66e4fdc51e2d89661e16626b7d824e203a64ef6'
    ...
    try: 
        ...
        messages.error(request, 'Network busy. Please try again in few minutes. Thanks')
    else:
        #create an order
